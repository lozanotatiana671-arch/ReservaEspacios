...
public class ReporteServlet extends HttpServlet {
    ...
    protected void doGet(HttpServletRequest request, HttpServletResponse response)
        ...
        String estadoReserva = request.getParameter("estado");  // Estados REALES de reservas
        ...
        try (Connection con = ConexionDB.getConnection()) {
            ...
            // =====================================================
            // 1. RESERVAS POR ESTADO (APROBADA, PENDIENTE…)
            // =====================================================
            ...
            if (tieneFecha)
                sqlEstado.append(" AND r.fecha BETWEEN ? AND ? ");
            ...
            if (tieneFecha)
                sqlRecurso.append(" AND r.fecha BETWEEN ? AND ? ");
            ...
            if (tieneFecha)
                sqlTipo.append(" AND r.fecha BETWEEN ? AND ? ");
            ...
            // =====================================================
            // 4. TABLA DE RECURSOS (NO SE MEZCLA CON RESERVAS)
            //    SE MANTIENE COMO ANTES (TIPO / ESTADO DEL RECURSO)
            // =====================================================
            StringBuilder sqlRecursos = new StringBuilder(
                "SELECT nombre, tipo, estado, capacidad, tarifa, ubicacion "
              + "FROM recursos WHERE 1=1 "
            );

            List<Object> params = new ArrayList<>();

            if (tipoEspacio != null && !tipoEspacio.isEmpty()) {
                sqlRecursos.append(" AND tipo = ? ");
                params.add(tipoEspacio);
            }

            sqlRecursos.append(" ORDER BY nombre ASC ");
